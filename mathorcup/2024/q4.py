...
def raw_sample_status(sample, current_solution, i1, i2):
    fixed_budget = 0.0
    fixed_truck = np.zeros(10, dtype=int)
    fixed_type_count = 0

    for i, cfg in enumerate(current_solution):
        if i in (i1, i2) or cfg is None:
            continue
        fixed_type_count += 1
        fixed_budget += cfg["budget"]
        fixed_truck[cfg["j"]] += cfg["k"]

    raw_budget = fixed_budget
    raw_truck = fixed_truck.copy()
    raw_type_count = fixed_type_count
    structure_bad = False
    ...
    for i in (i1, i2):
        ...
        for cfg in selected:
            ...
            raw_truck[cfg["j"]] += cfg["k"]

    type_bad = raw_type_count < 5
    budget_bad = raw_budget > 4000 + 1e-8
    truck_bad = np.any(raw_truck > N)
...
def solve_pair_by_sa(current_solution, i1, i2, seed):
    ...
    best_profit = evaluate_solution(best_solution)["profit"]
    ...
    for datum in sampleset.data(
        ...
        status = raw_sample_status(sample, current_solution, i1, i2)

        stats["total"] += occ
        stats["feasible"] += occ * int(status["feasible"])
        stats["structure"] += occ * int(status["structure"])
        stats["type"] += occ * int(status["type"])
        stats["budget"] += occ * int(status["budget"])
        stats["truck"] += occ * int(status["truck"])

        candidate = decode_pair_sample(sample, current_solution, i1, i2)
    ...
    return best_solution, best_profit, bit_count, stats
...
def pair_subqubo_descent(initial_solution, rng, start_id, max_outer_sweeps=4):
    ...
    for outer in range(max_outer_sweeps):
        ...
        for pair_id, (i1, i2) in enumerate(pairs):
            ...
            _, exact_profit = solve_pair_exact(
                current_solution,
                i1,
                i2,
            )
            ...
            candidate, sa_profit, bit_count, raw_stats = solve_pair_by_sa(
                current_solution,
                i1,
                i2,
                seed,
            ...
            diag["raw_total"] += raw_stats["total"]
            diag["raw_feasible"] += raw_stats["feasible"]
            diag["structure"] += raw_stats["structure"]
            diag["type"] += raw_stats["type"]
            diag["budget"] += raw_stats["budget"]
            diag["truck"] += raw_stats["truck"]
            ...
            exact_can_improve = exact_improvement > 1e-8
            sa_can_improve = sa_improvement > 1e-8

            if exact_can_improve:
            ...
            if pair_gap > diag["max_pair_gap"] + 1e-9:
                diag["max_pair_gap"] = pair_gap
                diag["max_gap_record"] = {
                    "outer": outer + 1,
                    "pair": (i1 + 1, i2 + 1),
...
def main():
    ...
    print(
        "罚权重 =",
        (LAMBDA_M, LAMBDA_B, LAMBDA_K, LAMBDA_T),
    )
    print(
        "reads / sweeps / starts =",
        PAIR_NUM_READS,
    ...
    for start_id in range(1, N_STARTS + 1):
        ...
        for key in (
            "pair_calls",
            "exact_improvable",
            "sa_found_improvement",
            "sa_missed_improvement",
            "sa_hit_exact_improvable",
            "raw_total",
            "raw_feasible",
            "structure",
            "type",
        ...
        if diag["max_pair_gap"] > total["max_pair_gap"]:
            total["max_pair_gap"] = diag["max_pair_gap"]
            total["max_gap_record"] = {
                "start": start_id,
                **diag["max_gap_record"],
            }
    ...
    print(
        "最好方案相对gap =",
        (MILP_BENCHMARK - best_profit) / MILP_BENCHMARK * 100,
        "%",
    )
    ...
    print(
        "SA对可改善pair的改善发现率 =",
        f"{total['sa_found_improvement']/exact_n:.2%}" if exact_n else "0.00%",
    )
    print(
        "SA对可改善pair的Exact命中率 =",
        f"{total['sa_hit_exact_improvable']/exact_n:.2%}" if exact_n else "0.00%",
    )
    print(
        "全部pair平均 Exact-SA gap =",
        total["sum_pair_gap"] / pair_n if pair_n else 0.0,
    )
    print(
        "仅Exact可改善pair的平均 Exact-SA gap =",
        total["sum_pair_gap_improvable"] / exact_n if exact_n else 0.0,
    )
    print("最大单次 Exact-SA gap =", total["max_pair_gap"])
    if total["max_gap_record"]:
        print("最大gap记录 =", total["max_gap_record"])

    raw_n = max(1, total["raw_total"])

    print("\n================ SA原始sample约束诊断 ================")
    print("原始sample总数 =", total["raw_total"])
    print("原始sample整体可行率 =", f"{total['raw_feasible']/raw_n:.2%}")
    print("同型号多配置违规率 =", f"{total['structure']/raw_n:.2%}")
    print("至少5型号违规率 =", f"{total['type']/raw_n:.2%}")
    print("预算违规率 =", f"{total['budget']/raw_n:.2%}")
    print("矿车库存违规率 =", f"{total['truck']/raw_n:.2%}")
